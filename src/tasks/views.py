...
class FetchTask(APIView):
    ...
    def get(self,request):
        id=request.user.id # user id from token.
        print('id',id)
        allTaskAssignedtoMe = Task.objects.filter(assigned_to=id).all()

        if not allTaskAssignedtoMe:
            return Response({'message':'No task assigned to you'},status=status.HTTP_400_BAD_REQUEST)
...
# now this has a issue like user role can updte the title and desciptoin alos
class UpdateTaskStatus(APIView):
    ...
    def put(self,request,id,*args, **kwargs):
        ...
        if "status" not in request.data or len(request.data) > 1:
            return Response({'message':'You can only update the status'},status=status.HTTP_400_BAD_REQUEST)
